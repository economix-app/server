...
@app.route("/api/generate_user_report", methods=["POST"])
@requires_admin
def generate_user_report_endpoint():
    ...
    if user.get("banned"):
        punishment_history.append(
            {
                "type": "Ban",
                "reason": user.get("banned_reason", "N/A"),
                "until": user.get("banned_until", "Permanent"),
            }
        )
    if user.get("muted"):
        punishment_history.append(
            {
                "type": "Mute",
                "until": user.get("muted_until", "Permanent"),
            }
        )
    ...
    pdf.cell(200, 10, txt=f"User Report: {username}", ln=True, align="C")
    pdf.ln(10)

    # General Info
    pdf.set_font("Arial", size=12)
    pdf.cell(200, 10, txt="General Information", ln=True, align="L")
    pdf.cell(200, 10, txt=f"Username: {user['username']}", ln=True, align="L")
    pdf.cell(
        200, 10, txt=f"Created At: {time.ctime(user['created_at'])}", ln=True, align="L"
    )
    pdf.cell(
        200, 10, txt=f"IP Address: {user.get('creation_ip', 'N/A')}", ln=True, align="L"
    )
    pdf.cell(
        200, 10, txt=f"Account Type: {user.get('type', 'user')}", ln=True, align="L"
    )
    pdf.cell(200, 10, txt=f"Tokens: {user.get('tokens', 0)}", ln=True, align="L")
    pdf.cell(200, 10, txt=f"Level: {user.get('level', 1)}", ln=True, align="L")
    pdf.cell(200, 10, txt=f"Experience: {user.get('exp', 0)}", ln=True, align="L")
    pdf.cell(200, 10, txt=f"Plan: {get_plan(username)}", ln=True, align="L")
    pdf.ln(10)

    # Punishment History
    pdf.cell(200, 10, txt="Punishment History", ln=True, align="L")
    if punishment_history:
        for punishment in punishment_history:
            pdf.cell(200, 10, txt=f"Type: {punishment['type']}", ln=True, align="L")
            pdf.cell(
                200,
                10,
                txt=f"Reason: {punishment.get('reason', 'N/A')}",
                ln=True,
                align="L",
            )
            pdf.cell(200, 10, txt=f"Until: {punishment['until']}", ln=True, align="L")
            pdf.ln(5)
    else:
        pdf.cell(200, 10, txt="No punishments found.", ln=True, align="L")
    pdf.ln(10)

    # History
    pdf.cell(200, 10, txt="User History", ln=True, align="L")
    if history:
        for entry in history:
            pdf.cell(200, 10, txt=f"Action: {entry['action']}", ln=True, align="L")
            pdf.cell(
                200,
                10,
                txt=f"Timestamp: {time.ctime(entry['timestamp'])}",
                ln=True,
                align="L",
            )
            pdf.cell(
                200,
                10,
                txt=f"Item ID: {entry.get('item_id', 'N/A')}",
                ln=True,
                align="L",
            )
            pdf.ln(5)
    else:
        pdf.cell(200, 10, txt="No history found.", ln=True, align="L")
    pdf.ln(10)
